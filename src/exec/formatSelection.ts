...
const selectAndReplace = (node: any, text:string) => {
  const range = document.createRange();
  const selection = document.getSelection();
  if (!node.parentNode || !selection) return;
...
const formatChildren = (
  format: string, node: any, from: number, to: number | undefined = undefined,
) => {
  ...
  for (let i = from; i < acTo; i++) {
    ...
    if (child.nodeType === 3) {
      ...
      selectAndReplace(child, unicodeFormat(child.data, format));
    } else if (child.nodeType === 1 && child.getAttribute('data-code')) continue;
    else formatChildren(format, child, 0);
  }
};
...
const formatSelection = (TextAreaElement: Node, format: string) => {
  ...
  if (fromNode === toNode) {
    if (fromNode.nodeType === 3) {
      const text = unicodeFormat(fromNode.data.substring(fromOffset, toOffset), format);
      document.execCommand('insertText', false, text);
    } else {
  ...
  } else formatChildren(format, fromNode, fromOffset);
  // eslint-disable-next-line no-plusplus
  for (let i = from + 1; i < to; i++) {
    const node:any = allChildren[i];
    if (node.nodeType === 3) selectAndReplace(node, unicodeFormat(node.data, format));
  }
  ...
  } else formatChildren(format, toNode, 0, toOffset);
};
