...
type FormatArraysObj = {
  [key: string]: StringArray;
}

type FormatsAlphabetsObj = {
  [alpha: string]: string;
}

type CharMap = {
  [normalChar: string]: string;
}

type FormatsMap = {
  [alpha:string]: CharMap
}

/* eslint-disable no-restricted-syntax */
const unicodeSplit = (str: string): StringArray => {
  const arr = [];
  for (const c of str) { arr.push(c); }
  return arr;
};

const formats: FormatsAlphabetsObj = {
...
const formatArrays:FormatArraysObj = {};
Object.keys(formats).forEach((key) => {
  formatArrays[key] = unicodeSplit(formats[key]);
});
const mapToNormal:CharMap = {};
const mapToFormat:FormatsMap = {};
...
const format = (text:string, font:string):string => {
